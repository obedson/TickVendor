...
def test_participation_metric_waits_for_all_required_verification_methods(tmp_path):
    engine = create_engine(f"sqlite:///{tmp_path / 'qualified-attendance.db'}")
    Base.metadata.create_all(engine)
    with Session(engine, expire_on_commit=False) as db:
        _owner, community, event = create_event_context(db)
        attendee = User(email="qualified-attendee@example.com", password_hash="hash")
        db.add(attendee)
        db.flush()
        event.required_verification_methods = ["qr", "organizer"]
        attendance = check_in(db, event, attendee, AttendanceCheckIn())
        assert attendance.status.value == "checked_in"
        assert user_metrics(db, attendee.id, community.id)["attendance_count"] == 0
        db.add_all([
            AttendanceVerification(
        ...
        db.commit()
        assert user_metrics(db, attendee.id, community.id)["attendance_count"] == 1
    engine.dispose()
