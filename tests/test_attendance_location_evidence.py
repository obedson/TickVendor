...
from src.models import Attendance, AuditLog, ImpactTransaction, TicketStatus
...
def test_failed_location_survives_request_rollback_and_can_retry(tmp_path, point, outcome):
    engine = make_db(tmp_path)
    with Session(engine, expire_on_commit=False) as db:
        organizer, buyer, _, outsider, _, event = setup(db)
        _, _, ticket = issue(db, event, buyer)
        with pytest.raises(HTTPException) as error:
            self_check_in(db, event.id, ticket.id, buyer, point)
        ...
        assert entry.metadata_json['outcome'] == outcome
        row = attendance_roster(event.id, db, organizer, limit=100, offset=0)[0]
        evidence = row['latest_location_attempt']
        assert evidence['latitude'] == float(point.latitude)
        assert evidence['longitude'] == float(point.longitude)
...
def test_http_failed_attempt_then_success_and_checkout(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        failed = client.post(path + '/check-in', headers=buyer_headers,
        ...
        assert client.post(path + '/check-in', headers=buyer_headers, json=point).json() == accepted.json()
