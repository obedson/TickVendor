...
- The production image starts through `python scripts/start_web.py`, which runs `python -m alembic upgrade head` and only then replaces itself with Uvicorn. Render Web Service Start Command should be `python scripts/start_web.py`; this is the setting that was missing in the affected deployment.
...
Registration/email failure acceptance: follow the Render/Brevo procedure in `docs/EXTERNAL_VERIFICATION_RUNBOOK.md`. Render staging SMTP delivery is externally verified through Brevo to Gmail on port `2525` from sender `noreply@solafeed.com`. Registration sends verification before commit; relay failure returns HTTP 503 and rolls back the user, profile, session, and token. Verification emails use a configured-`FRONTEND_URL` link/button, and the frontend exposes resend plus invalid/expired/already-used states without claiming verification.
