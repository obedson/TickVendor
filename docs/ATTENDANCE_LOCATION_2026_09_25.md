...
- The browser requests up to three fresh high-accuracy fixes when an event radius is available. It stops when sufficient accuracy is obtained, otherwise submits the most accurate reading. Permission denial always stops. Sampling is bounded to a user action; no continuous tracking. This cannot guarantee device precision.
- Follow-up after inspecting the NIPOST enum webform: temporary unavailable/timeout failures are now retried up to three times with a 15-second window per request and a one-second pause. A confident in-radius reading is GPS verified; an in-radius reading whose accuracy is missing or wider than the radius creates a pending, flagged attendance record for organizer verification. Its ticket remains active and no reward is awarded until an authorized organizer confirms it. An outside-radius reading remains rejected. Organizer confirmation is a real audited override, marks the ticket used once and can satisfy required attendance verification; a repeated participant request remains idempotent.
