...
## Organizer least-privilege / RBAC hardening reconciliation â€” 2026-09-19
...
- `python -m pytest tests/test_organizer_least_privilege.py -q` â€” **45 passed**, 2 warnings.
- Related backend regression batch â€” **28 passed**, 2 warnings.
- Broader backend event/task/ticket regression batch â€” **81 passed**, 2 warnings.
- `python -m pytest tests/test_ticket_type_api.py -q` â€” **7 passed**, 2 warnings.
- Full backend, `python -m pytest -q` â€” **403 passed**, 6 warnings.
- Ruff across all changed Python files â€” all checks passed.
- Frontend: `npm run build` passed; `npm run lint` passed; `node scripts/test-management-layout.mjs` â€” **28 passed, 0 failed**.
- `git diff --check` â€” no whitespace or conflict-marker errors; only the working copy's existing LFâ†’CRLF warnings.
