...
test('organizer verifies submitted task evidence through the browser', async ({ browser }) => {
  ...
  await participantPage.getByRole('button', { name: 'Tasks', exact: true }).click();
...
await expect(
  organizerPage.getByText('Browser-reviewed evidence.')
).toBeVisible();
...
await expect(
  organizerPage.getByRole('status').filter({
    hasText: 'Task verified and points awarded.',
  })
).toBeVisible();
