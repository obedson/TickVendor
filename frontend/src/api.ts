...
async function responseMessage(response: Response, fallback: string): Promise<string> { const contentType = response.headers.get('content-type') || ''; if (contentType.includes('application/json')) { const body = await response.json().catch(() => null); if (typeof body?.detail === 'string') return body.detail; if (typeof body?.message === 'string') return body.message; if (typeof body?.error?.message === 'string') { const first = body.error.details?.[0]; const field = Array.isArray(first?.loc) ? first.loc.filter((part: unknown) => part !== 'body').join('.') : ''; const detail = typeof first?.msg === 'string' ? first.msg : ''; return [body.error.message, field && detail ? `${field}: ${detail}` : detail].filter(Boolean).join(' '); } } return fallback; }
