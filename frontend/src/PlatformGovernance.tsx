...
type Item = { id: string; name: string; suspended: boolean; role?: string; email?: string; username?: string; is_email_verified?: boolean; member_count?: number; membership_access?: string; is_public?: boolean; community_name?: string; status?: string; lifecycle_status?: 'draft' | 'pending_review' | 'active' | 'rejected' | 'suspended'; submitted_by?: { id: string; name: string; email: string }; reviewed_at?: string; review_reason?: string; owner?: { name: string }; description?: string; membership_offset?: number; memberships_has_more?: boolean; active_admin_count?: number; memberships?: Membership[] };
...
export function PlatformGovernance({ token, userRole, categories }: { token: string; userRole: string; categories: ReactNode }) {
  ...
  const load = async () => {
    ...
    try {
      if (section === 'history') setHistory(await apiJson<History[]>(`${base}/history?offset=${offset}${historyTarget ? `&target_id=${historyTarget}` : ''}`, {}, live()));
        ...
        if (section === 'communities' && status) query.set('status', status);
  ...
  return <section className="management-screen"><div className="page-header"><div><p className="eyebrow">Platform</p><h1>Platform Administration</h1></div></div>
    ...
    {section === 'overview' && <div className="panel"><h2>Governance and oversight</h2><p>Review community applications, appoint verified administrators, manage membership authority and apply reversible policy suspensions. Every governance change records a reason and history. Financial and participation evidence is retained.</p></div>}
    ...
    {['users', 'communities', 'content'].includes(section) && <>
      <form className="panel" onSubmit={event => { event.preventDefault(); if (offset) setOffset(0); else void load(); }}>
        ...
        {section === 'communities' && <label>Application lifecycle<select value={status} onChange={event => setStatus(event.target.value)}><option value="">All</option>{['draft', 'pending_review', 'active', 'rejected', 'suspended'].map(value => <option key={value}>{value.replaceAll('_', ' ')}</option>)}</select></label>}
      ...
      {loading ? <p role="status">Loading…</p> : <div className="definition-list">{items.map(item => <article key={item.id} className="definition-card"><div><h2>{item.name}</h2><p>{item.email || item.community_name} {item.owner ? `· ${item.owner.name}` : ''}</p>{item.lifecycle_status ? <span className="chip">{item.lifecycle_status.replaceAll('_', ' ')}</span> : <span className="chip">{item.suspended ? 'Suspended' : 'Not suspended'}</span>}{item.status && <span className="chip">{item.status}</span>}</div><button className="secondary" onClick={() => void inspect(item)}>Inspect {item.name}</button></article>)}{!items.length && <p>No matching records.</p>}</div>}
      {selected && <section className="panel" data-governance-inspection><h2>{selected.name}</h2><p>{selected.description}</p>{selected.email && <p>{selected.email} · @{selected.username} · {selected.role} · Email {selected.is_email_verified ? 'verified' : 'unverified'}</p>}
        {currentKind === 'community' && <p>{selected.lifecycle_status?.replaceAll('_', ' ')} · {selected.is_public ? 'Public' : 'Private'} · {selected.membership_access?.replaceAll('_', ' ')} · {selected.member_count} active members{selected.submitted_by ? ` · submitted by ${selected.submitted_by.name}` : ''}</p>}
        {currentKind === 'community' && selected.lifecycle_status === 'pending_review' && <div className="editor-panel"><h3>Review application</h3><p>Approval activates the community and organization and appoints one verified initial Community Admin.</p><label>Find an existing verified user<input value={userQuery} onChange={event => setUserQuery(event.target.value)} /></label><button className="secondary" onClick={() => void searchUsers()}>Search users</button><label>Initial Community Admin<select value={assignee || selected.submitted_by?.id || ''} onChange={event => setAssignee(event.target.value)}><option value="">Select a verified user</option>{selected.submitted_by && <option value={selected.submitted_by.id}>{selected.submitted_by.name} — applicant</option>}{users.filter(item => !item.suspended && item.is_email_verified).map(item => <option key={item.id} value={item.id}>{item.name} — {item.email}</option>)}</select></label><div className="form-actions"><button disabled={!(assignee || selected.submitted_by?.id)} onClick={() => setPending({ title: `Approve ${selected.name}`, consequence: 'Activates the community and grants its initial administrator full community governance authority.', path: `${base}/community/${selected.id}/review`, body: { approve: true, initial_admin_user_id: assignee || selected.submitted_by?.id } })}>Approve application</button><button className="secondary" onClick={() => setPending({ title: `Reject ${selected.name}`, consequence: 'The application remains in governance history and does not become discoverable or operational.', path: `${base}/community/${selected.id}/review`, body: { approve: false } })}>Reject application</button></div></div>}
        {!(currentKind === 'user' && selected.role === 'super_admin') && !(currentKind === 'community' && ['draft', 'pending_review', 'rejected'].includes(selected.lifecycle_status || '')) && <button className="secondary" onClick={() => setPending({ title: `${selected.suspended ? 'Restore' : 'Suspend'} ${selected.name}`, consequence: 'Suspension blocks ordinary access or content use. Existing records remain; restoration is reversible.', path: `${base}/${currentKind}/${selected.id}/moderation`, body: { suspended: !selected.suspended } })}>{selected.suspended ? 'Restore' : 'Suspend'}</button>}
        {selected.memberships && <><h3>Memberships and administrators</h3>{currentKind === 'community' && <p>{selected.active_admin_count} active Community Admins. Assign a verified replacement before removing the final Admin.</p>}<div className="definition-list">{selected.memberships.map(member => <article className="definition-card" key={member.membership_id}><div><strong>{currentKind === 'user' ? member.community_name : member.name}</strong><p>{member.role} · {member.status}</p></div>{currentKind === 'community' && ['active', 'suspended'].includes(member.status) && <div className="form-actions">{(['member', 'organizer', 'admin'] as const).filter(role => role !== member.role).map(role => <button key={role} className="secondary" onClick={() => setPending({ title: `Change ${member.name} to ${role}`, consequence: 'This changes community authority, not the platform role. The final active Admin cannot be removed.', path: `${base}/community/${selected.id}/memberships/${member.membership_id}`, method: 'PATCH', body: { action: 'role_changed', role } })}>Make {role}</button>)}<button className="secondary" onClick={() => setPending({ title: `Change membership access for ${member.name}`, consequence: 'History remains; suspended memberships cannot use community authority. The final active Admin cannot be deactivated.', path: `${base}/community/${selected.id}/memberships/${member.membership_id}`, method: 'PATCH', body: { action: member.status === 'active' ? 'deactivated' : 'activated' } })}>{member.status === 'active' ? 'Deactivate' : 'Activate'}</button></div>}</article>)}</div><div className="form-actions"><button className="secondary" disabled={!selected.membership_offset} onClick={() => void inspect(selected, Math.max(0, (selected.membership_offset || 0) - 50))}>Previous memberships</button><button className="secondary" disabled={!selected.memberships_has_more} onClick={() => void inspect(selected, (selected.membership_offset || 0) + 50)}>Next memberships</button></div></>}
        {currentKind === 'community' && ['active', 'suspended'].includes(selected.lifecycle_status || '') && <div className="editor-panel"><h3>Assign Community Admin</h3><label>Find an existing verified user by email/username<input value={userQuery} onChange={event => setUserQuery(event.target.value)} /></label><button className="secondary" onClick={() => void searchUsers()}>Search users</button><label>User<select value={assignee} onChange={event => setAssignee(event.target.value)}><option value="">Select a verified user</option>{users.filter(item => !item.suspended && item.is_email_verified).map(item => <option key={item.id} value={item.id}>{item.name} — {item.email}</option>)}</select></label><button disabled={!assignee} onClick={() => setPending({ title: `Assign ${users.find(item => item.id === assignee)?.name} as Admin`, consequence: `Grants active administrator authority in ${selected.name}. The platform role does not change.`, path: `${base}/community/${selected.id}/administrators`, body: { user_id: assignee } })}>Assign Community Admin</button></div>}
